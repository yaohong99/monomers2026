...
def get_property(file1, file2, file3, file4,
    ...
    plt.scatter(T1, Z1, color='b', marker='o', label='x-direction(C=0.123 mol/L)')
    plt.scatter(T2, Z2, color='magenta', marker='s', label='z-direction(C=0.123 mol/L)')
    plt.scatter(X1, Y1, color='r', marker='^', label='x-direction(C=1.293 mol/L)')
    plt.scatter(X2, Y2, color='orange', marker='v', label='z-direction(C=1.293 mol/L)')

    if fit_ranges is not None:
        datasets = [
            (T1, Z1, 'b', '0.123-x'),
            (T2, Z2, 'magenta', '0.123-z'),
            (X1, Y1, 'r', '1.293-x'),
            (X2, Y2, 'orange', '1.293-z')
        ]
        ...
        for i, (x, y, color, name) in enumerate(datasets):
            if fit_ranges[i] is not None:
                ...
                plt.plot(x_fit, k*x_fit + b, '--', color=color, linewidth=2.5,
